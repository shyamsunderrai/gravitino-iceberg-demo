...
################################################################################
# ════════════════════════════════════════════════════════════════════════════
# PHASE 5.5: Pre-register SPIFFE users in Gravitino H2 database
#
# WHY THIS MUST HAPPEN BEFORE OAUTH2 IS ENABLED:
#
#   Gravitino's AuthorizationUtils.checkCurrentUser requires the calling
#   user to exist as a record in the metalake — even for serviceAdmins.
#   Once OAuth2 is enabled in Phase 6, every JWT sub must already be in
#   USER_META or Gravitino returns 403.
#
# WHY DIRECT H2 INSERT (NOT THE GRAVITINO API):
#
#   The Gravitino user management API (/api/metalakes/{ml}/users) requires
#   authorization.enable=true to work (returns 405 otherwise). But with
#   authorization.enable=true AND oauth auth, every request needs a JWT
#   whose sub already exists in USER_META — circular dependency.
#
#   Solution: insert rows directly into H2 via H2 Shell from inside the
#   Gravitino pod, using AUTO_SERVER=TRUE so our shell connection shares
#   the same H2 instance as Gravitino. Gravitino's Flyway migrations run
#   at startup so we must insert AFTER startup (not while pod is scaled down).
#
# AUDIT_INFO: Gravitino deserializes this CLOB as JSON. Must be valid JSON
#   with double-quoted keys. The createTime field must be ISO-8601.
# ════════════════════════════════════════════════════════════════════════════
################################################################################

echo ""
echo "════════════════════════════════════════════════════════════════════════════"
echo " Phase 5.5: Pre-registering SPIFFE users directly into Gravitino H2 DB"
echo "════════════════════════════════════════════════════════════════════════════"

GRAVITINO_POD=$(kubectl get pod -n "${NAMESPACE}" -l app.kubernetes.io/name=gravitino \
  -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || \
  kubectl get pods -n "${NAMESPACE}" | grep "^gravitino-" | grep Running | grep -v spark | awk '{print $1}' | head -1)

if [ -z "${GRAVITINO_POD}" ]; then
  error "Could not find running Gravitino pod. Is Gravitino deployed?"
  exit 1
fi
info "Gravitino pod: ${GRAVITINO_POD}"

# Get the metalake ID from H2 (generated by Gravitino on metalake creation)
METALAKE_ID=$(kubectl exec -n "${NAMESPACE}" "${GRAVITINO_POD}" -- sh -c '
  echo "SELECT METALAKE_ID FROM METALAKE_META WHERE METALAKE_NAME='"'"'poc_layer'"'"';" | \
  java -cp /root/gravitino/libs/h2-1.4.200.jar org.h2.tools.Shell \
    -url "jdbc:h2:file:/root/gravitino/data/jdbc;MODE=MYSQL;DB_CLOSE_ON_EXIT=FALSE;AUTO_SERVER=TRUE" \
    -user gravitino -password gravitino 2>/dev/null | grep -E "^[0-9]+" | head -1 | tr -d " "
')

if [ -z "${METALAKE_ID}" ]; then
  error "Could not read metalake ID from H2. Is the poc_layer metalake created? Run 02-register-catalogs.sh first."
  exit 1
fi
info "Metalake ID: ${METALAKE_ID}"

# Build SQL: delete any stale rows first, then insert fresh with correct JSON AUDIT_INFO
AUDIT_JSON='{"creator":"bootstrap","createTime":"2026-04-08T00:00:00.000Z"}'

kubectl exec -n "${NAMESPACE}" "${GRAVITINO_POD}" -- sh -c "
echo \"DELETE FROM USER_META;
COMMIT;
INSERT INTO USER_META(USER_ID,USER_NAME,METALAKE_ID,AUDIT_INFO,CURRENT_VERSION,LAST_VERSION,DELETED_AT) VALUES(1,'spiffe://gravitino.demo/admin',${METALAKE_ID},'{\\\"creator\\\":\\\"bootstrap\\\",\\\"createTime\\\":\\\"2026-04-08T00:00:00.000Z\\\"}',1,1,0);
INSERT INTO USER_META(USER_ID,USER_NAME,METALAKE_ID,AUDIT_INFO,CURRENT_VERSION,LAST_VERSION,DELETED_AT) VALUES(2,'spiffe://gravitino.demo/spark-oc-writer',${METALAKE_ID},'{\\\"creator\\\":\\\"bootstrap\\\",\\\"createTime\\\":\\\"2026-04-08T00:00:00.000Z\\\"}',1,1,0);
INSERT INTO USER_META(USER_ID,USER_NAME,METALAKE_ID,AUDIT_INFO,CURRENT_VERSION,LAST_VERSION,DELETED_AT) VALUES(3,'spiffe://gravitino.demo/spark-ac-writer',${METALAKE_ID},'{\\\"creator\\\":\\\"bootstrap\\\",\\\"createTime\\\":\\\"2026-04-08T00:00:00.000Z\\\"}',1,1,0);
INSERT INTO USER_META(USER_ID,USER_NAME,METALAKE_ID,AUDIT_INFO,CURRENT_VERSION,LAST_VERSION,DELETED_AT) VALUES(4,'spiffe://gravitino.demo/gravitino',${METALAKE_ID},'{\\\"creator\\\":\\\"bootstrap\\\",\\\"createTime\\\":\\\"2026-04-08T00:00:00.000Z\\\"}',1,1,0);
COMMIT;
SELECT USER_ID,USER_NAME FROM USER_META;\" | java -cp /root/gravitino/libs/h2-1.4.200.jar org.h2.tools.Shell \
  -url 'jdbc:h2:file:/root/gravitino/data/jdbc;MODE=MYSQL;DB_CLOSE_ON_EXIT=FALSE;AUTO_SERVER=TRUE' \
  -user gravitino -password gravitino 2>&1 | grep -E 'Update count|USER_ID|spiffe|rows'
"

success "SPIFFE users inserted into Gravitino H2 database"
